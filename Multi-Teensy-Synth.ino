...
// Menu encoder pins now defined in MenuNavigation.h

// Engine and menu variables (now defined in MenuNavigation.h)
EngineType currentEngine = ENGINE_VA; // Default to VA engine
ParentMenuState currentParentMenu = PARENT_PARAMETERS; // Start in parameters like original
bool inParentMenu = false;

// DX7 FM synthesis components
AudioSynthDexed          dexed(6, AUDIO_SAMPLE_RATE);  // DX7 FM synthesis engine
bool dx7BankLoaded = false;
int currentDX7Preset = 0;
int dx7BrowseIndex = 0;  // For browsing presets without loading
uint8_t dx7EngineType = 0; // MSFA engine (warmest, no pops)

// Engine mixing - now supports 3 engines 
AudioMixer4              engineMix;      // Mix VA, Juno, and DX7 engines
int engineBrowseIndex = 0; // For browsing engines without switching (0=VA, 1=Juno, 2=DX7)
...
float allParameterValues[41] = {
  ...
  1.000, // Chorus Bypass (31) - 1.0 = bypassed (start with chorus off)
  0.270, // Chorus Rate (32) - Juno-60 chorus speed  
  0.600, // Chorus Depth (33) - Deeper for chorus effect
  1.000, // Reverb Bypass (34) - 1.0 = bypassed (start with reverb off)
  0.300, // Reverb Size (35)
  0.500, // Reverb Hi Damp (36)
  0.000, // Reverb Lo Damp (37)
  0.700, // Reverb Lowpass (38)
  0.650  // Reverb Diffusion (39)
};

// Audio synthesis
// VOICES now defined in config.h
// FLANGE_DELAY_LENGTH now defined in config.h
...
// Juno-60 specific audio components
AudioSynthWaveformSine   pwmLfo;              // PWM LFO for pulse width modulation
AudioFilterBiquad        junoHPF[VOICES];     // High-pass filter per voice (Juno-60 style)
...
// Shared flange delay buffers (single pair for all engines)
...
// Note: Juno-60 engine reuses VA oscillators with different settings
...
// Juno-60 high-pass filter connections (parallel to main filter chain)
...
float filtAttack = 100, filtSustain = 0.5, filtDecay = 2500, filtRelease = 2500; // Better default filter envelope
...
float chorusRate = 0.27; // Chorus modulation rate (0-1, maps to 0-1 Hz) - Juno-60 chorus speed
...
// Control system
  ...
  // Dummy LCD object to prevent compilation errors while converting
  LiquidCrystal_I2C lcd(0x27, 16, 2);
#endif
int menuIndex = 0;
bool inMenu = false;
...
// Preset system (struct now defined in MenuNavigation.h)
...
// Juno-60 presets structure 
// JunoPreset struct now defined in MenuNavigation.h

// Classic Juno-60 style presets (resonance reduced to prevent feedback)
...
void setVoiceFrequencies(int voiceNum, float baseFreq, float pitchWheelMultiplier) {
  if (currentEngine == ENGINE_JUNO) {
    // Juno-60 style: Main DCO with PWM + sub oscillator (one octave down)
...
void switchEngine(EngineType newEngine) {
  ...
  // Update engine routing to shared effects chain
  ...
  switch(currentEngine) {
    ...
    case ENGINE_JUNO:
      ...
      // Apply Juno-60 specific parameter constraints
      applyJunoParameterMapping();
      Serial.println("Juno-60 engine configured with authentic parameters");
      break;
    case ENGINE_DX7:
  ...
  const char* engineNames[] = {"VA", "Juno-60", "DX7"};
...
void loadDX7Preset(int presetNum) {
  ...
  dexed.loadVoiceParameters(progmem_bank[0][presetNum]);
  ...
  memcpy(voice_name, &progmem_bank[0][presetNum][144], 10);
...
// Display helper functions moved to MenuNavigation.cpp
...
//     sgt15000_1.volume(1);
  ...
  // Initialize Juno-60 specific components
  Serial.println("Initializing Juno-60 components...");
  
  // PWM LFO initialization
  pwmLfo.frequency(0.5);      // Slow PWM sweep by default
  pwmLfo.amplitude(1.0);
  
  // High-pass filters for Juno voices (removes low-end mud, Juno-60 characteristic)
  for (int v = 0; v < VOICES; v++) {
  ...
  double s_freq = chorusRate;           // LFO frequency - Juno-60 chorus speed
  
  // Initialize shared flangers with Juno-60 chorus settings
  ...
  Serial.println("Juno-60 components initialized");
...
// Apply Juno-60 specific parameter mapping
void applyJunoParameterMapping() {
  if (currentEngine != ENGINE_JUNO) return;
  
  // Juno-60 oscillator configuration
  ...
  // Force Juno-60 appropriate ranges
  osc1Range = 1.0;  // No range control on Juno-60
  osc2Range = 0.5;  // Sub-oscillator is always -1 octave
  osc3Range = 1.0;  // Standard range
  
  // Force Juno-60 fine tune off
  osc1Fine = 1.0;   // No fine tune on Juno-60
  osc2Fine = 1.0;   // No fine tune on Juno-60
  osc3Fine = 1.0;   // No fine tune on Juno-60
  
  // Juno-60 authentic DCO behavior:
  // DCO Section: PWM OR Sawtooth (not both)
  // Sub-oscillator: Square wave -1 octave (independent)
  
  // Juno-60 DCO waveform selection logic
  ...
  // Force osc3 off for authentic 2-oscillator Juno-60 behavior
  vol3 = 0.0;
  osc3Wave = WAVEFORM_SQUARE;
  
  // Juno-60 envelope: Single ADSR shared by filter and amp
