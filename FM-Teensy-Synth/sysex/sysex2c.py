...
def help_message():
	print(progname+" [--decode] [--auto] [<sysex1> ... <sysexn>]")
	print()
	print("Convert DX7 sysex banks to C arrays for Multi-Teensy-Synth")
	print()
	print("Options:")
	print("  --decode    Convert to unpacked 156-byte format (recommended)")
	print("              Without this flag, uses packed 128-byte format")
...
# Check if no arguments provided (default mode)
if len(sys.argv) == 0:
	default_mode = True
...
while len(sys.argv) > 0 and sys.argv[0].startswith("--"):
	if sys.argv[0] == "--decode":
		...
		print(f"Unknown option: {sys.argv[0]}")
		help_message()
		exit(1)

# Handle auto-scan mode or default behavior
if auto_scan or default_mode or len(sys.argv) == 0:
	# Find all .syx files in current directory
	syx_files = glob.glob("*.syx") + glob.glob("*.SYX")
	syx_files.sort()  # Sort for consistent output
	if not syx_files:
		if auto_scan:
			print("Error: No .syx files found in current directory")
		else:
			print("Error: No .syx files found in current directory")
			print("Usage: python3 sysex2c.py [--decode] [--auto] [file1.syx ...]")
		exit(1)
	sys.argv = syx_files
	auto_scan = True  # Set auto_scan flag for default behavior
	decode = True     # Default to decoded format
	...
	if default_mode:
		...
		print(f"// Auto-detected {len(syx_files)} sysex files: {', '.join(syx_files)}", file=sys.stderr)

print(f"""
//
// File generated with sysex2c.py
// Source files: {', '.join([os.path.basename(f) for f in sys.argv])}
//
...
if(decode==True):
	print("extern uint8_t progmem_bank[%d][32][156];" % int(len(sys.argv)))
	print("")
	print("#ifdef DX7_IMPLEMENTATION")
	print("uint8_t progmem_bank[%d][32][156] PROGMEM =\n{" % int(len(sys.argv)))
else:
	print("extern uint8_t progmem_bank[%d][32][128];" % int(len(sys.argv)))
	print("")
	print("#ifdef DX7_IMPLEMENTATION")
...
print("")
print("#endif // DX7_IMPLEMENTATION")
